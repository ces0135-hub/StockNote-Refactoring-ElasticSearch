...
class ServiceManager:
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'stocknote_be_2', self.services['stocknote_be_2']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8090 -e TZ=Asia/Seoul -v /dockerProjects/stock_note/volumes/gen:/gen --pull always ghcr.io/backendschoolplus3th/stocknote_be")
