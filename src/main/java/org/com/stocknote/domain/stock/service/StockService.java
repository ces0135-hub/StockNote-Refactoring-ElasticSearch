...
public class StockService {
    ...
    public StockPriceResponse getStockPrice(String stockCode) {
        String baseUrl = "https://openapivts.koreainvestment.com:29443"; // Change to production URL
                                                                         // if needed
        ...
        try {
            ResponseEntity<StockPriceResponse> response =
                    restTemplate.exchange(url, HttpMethod.GET, entity, StockPriceResponse.class);

            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                return response.getBody();
            } else {
                throw new RuntimeException(
                        "Failed to get stock price. Status code: " + response.getStatusCode());
            }
    ...
    public StockDailyResponse getStockPrices(String stockCode, PeriodType periodType,
        ...
        String url = UriComponentsBuilder.fromHttpUrl(baseUrl + endpoint)
                ...
                .queryParam("FID_ORG_ADJ_PRC", "0").build(false).toUriString();
        ...
        try {
            ...
            log.error("API Error for stockCode {} with period {}: {}", stockCode, periodType,
                    e.getMessage());
    ...
    // 일별 데이터 조회
    public StockDailyResponse getDailyStockPrices(String stockCode, LocalDate startDate,
            LocalDate endDate) {
        return getStockPrices(stockCode, PeriodType.DAILY, startDate, endDate);
    }

    // 주별 데이터 조회
    public StockDailyResponse getWeeklyStockPrices(String stockCode, LocalDate startDate,
            LocalDate endDate) {
        return getStockPrices(stockCode, PeriodType.WEEKLY, startDate, endDate);
    }

    // 월별 데이터 조회
    public StockDailyResponse getMonthlyStockPrices(String stockCode, LocalDate startDate,
            LocalDate endDate) {
        return getStockPrices(stockCode, PeriodType.MONTHLY, startDate, endDate);
    }

    // 연간 데이터 조회
    public StockDailyResponse getYearlyStockPrices(String stockCode, LocalDate startDate,
            LocalDate endDate) {
    ...
    public StockTimeResponse getTimeStockPrices(String stockCode) {
        ...
        String url = UriComponentsBuilder.fromHttpUrl(baseUrl + endpoint)
                .queryParam("FID_COND_MRKT_DIV_CODE", "J").queryParam("FID_INPUT_ISCD", stockCode)
                .queryParam("FID_INPUT_HOUR_1", formattedTime).build(false).toUriString();
        ...
        try {
            ...
            // ObjectMapper로 변환
            ObjectMapper objectMapper = new ObjectMapper();
            StockTimeResponse response =
                    objectMapper.readValue(rawResponse.getBody(), StockTimeResponse.class);
