...
public class NoteResponse {
  ...
  private int amount;
  private int price;
  private String portfolioName; // Portfolio 엔티티 대신 필요한 필드만 추출
  private String createdAt; // 거래 일자 추가

  public static NoteResponse from(Note note) {
    return NoteResponse.builder()
        .type(note.getType())
        .stockName(note.getStock().getName())
        .market(note.getStock().getMarket())
        .amount(note.getAmount())
        .price(note.getPrice())
        .portfolioName(note.getPortfolio().getName())
